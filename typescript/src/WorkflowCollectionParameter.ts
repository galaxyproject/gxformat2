...
/**
 * Auto-generated class implementation for https://galaxyproject.org/gxformat2/v19_09#WorkflowCollectionParameter
 */
export class WorkflowCollectionParameter extends Saveable implements Internal.WorkflowCollectionParameterProperties {
  ...
  constructor ({loadingOptions, extensionFields, id, label, doc, default_, position, optional, format, type, collection_type} : {loadingOptions?: LoadingOptions} & Internal.WorkflowCollectionParameterProperties) {
  ...
  /**
   * Used to construct instances of {@link WorkflowCollectionParameter }.
   *
   * @param __doc                           Document fragment to load this record object from.
   * @param baseuri                         Base URI to generate child document IDs against.
   * @param loadingOptions                  Context for loading URIs and populating objects.
   * @param docRoot                         ID at this position in the document (if available)
   * @returns                               An instance of {@link WorkflowCollectionParameter }
  ...
  static override async fromDoc (__doc: any, baseuri: string, loadingOptions: LoadingOptions,
    ...
    let type
    try {
      type = await loadField(_doc.type, LoaderInstances.typedslGalaxyDataCollectionTypeLoader2,
        baseuri, loadingOptions)
    } catch (e) {
      if (e instanceof ValidationException) {
        __errors.push(
    ...
    for (const [key, value] of Object.entries(_doc)) {
      if (!WorkflowCollectionParameter.attr.has(key)) {
        if ((key as string).includes(':')) {
          ...
          __errors.push(
            ...
            expected one of: \`label\`,\`doc\`,\`id\`,\`default\`,\`position\`,\`optional\`,\`format\`,\`type\`,\`collection_type\``)
          )
          break
        }
      }
    ...
    if (__errors.length > 0) {
      throw new ValidationException("Trying 'WorkflowCollectionParameter'", __errors)
    }

    const schema = new WorkflowCollectionParameter({
  ...
  static attr: Set<string> = new Set(['label','doc','id','default','position','optional','format','type','collection_type'])
}
