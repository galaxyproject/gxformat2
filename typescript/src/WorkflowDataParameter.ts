...
/**
 * Auto-generated class implementation for https://galaxyproject.org/gxformat2/v19_09#WorkflowDataParameter
 */
export class WorkflowDataParameter extends Saveable implements Internal.WorkflowDataParameterProperties {
  ...
  type?: Internal.GalaxyDataType | undefined


  constructor ({loadingOptions, extensionFields, id, label, doc, default_, position, optional, format, type} : {loadingOptions?: LoadingOptions} & Internal.WorkflowDataParameterProperties) {
  ...
  /**
   * Used to construct instances of {@link WorkflowDataParameter }.
   *
   * @param __doc                           Document fragment to load this record object from.
   * @param baseuri                         Base URI to generate child document IDs against.
   * @param loadingOptions                  Context for loading URIs and populating objects.
   * @param docRoot                         ID at this position in the document (if available)
   * @returns                               An instance of {@link WorkflowDataParameter }
  ...
  static override async fromDoc (__doc: any, baseuri: string, loadingOptions: LoadingOptions,
    ...
    if ('type' in _doc) {
      try {
        type = await loadField(_doc.type, LoaderInstances.typedslunionOfGalaxyDataTypeLoaderOrundefinedtype2,
          baseuri, loadingOptions)
      } catch (e) {
        if (e instanceof ValidationException) {
          __errors.push(
    ...
    for (const [key, value] of Object.entries(_doc)) {
      if (!WorkflowDataParameter.attr.has(key)) {
        if ((key as string).includes(':')) {
          ...
          __errors.push(
            ...
            expected one of: \`label\`,\`doc\`,\`id\`,\`default\`,\`position\`,\`optional\`,\`format\`,\`type\``)
          )
          break
        }
      }
    ...
    if (__errors.length > 0) {
      throw new ValidationException("Trying 'WorkflowDataParameter'", __errors)
    }

    const schema = new WorkflowDataParameter({
  ...
  static attr: Set<string> = new Set(['label','doc','id','default','position','optional','format','type'])
}
