...
/**
 * Auto-generated class implementation for https://galaxyproject.org/gxformat2/v19_09#BaseInputParameter
 */
export class BaseInputParameter extends Saveable implements Internal.BaseInputParameterProperties {
  ...
  constructor ({loadingOptions, extensionFields, id, label, doc, default_, position, optional} : {loadingOptions?: LoadingOptions} & Internal.BaseInputParameterProperties) {
  ...
  /**
   * Used to construct instances of {@link BaseInputParameter }.
   *
   * @param __doc                           Document fragment to load this record object from.
   * @param baseuri                         Base URI to generate child document IDs against.
   * @param loadingOptions                  Context for loading URIs and populating objects.
   * @param docRoot                         ID at this position in the document (if available)
   * @returns                               An instance of {@link BaseInputParameter }
  ...
  static override async fromDoc (__doc: any, baseuri: string, loadingOptions: LoadingOptions,
    ...
    for (const [key, value] of Object.entries(_doc)) {
      if (!BaseInputParameter.attr.has(key)) {
        if ((key as string).includes(':')) {
          ...
          __errors.push(
            ...
            expected one of: \`label\`,\`doc\`,\`id\`,\`default\`,\`position\`,\`optional\``)
          )
          break
        }
      }
    ...
    if (__errors.length > 0) {
      throw new ValidationException("Trying 'BaseInputParameter'", __errors)
    }

    const schema = new BaseInputParameter({
  ...
  static attr: Set<string> = new Set(['label','doc','id','default','position','optional'])
}
