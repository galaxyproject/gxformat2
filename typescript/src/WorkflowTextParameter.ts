...
/**
 * Auto-generated class implementation for https://galaxyproject.org/gxformat2/v19_09#WorkflowTextParameter
 */
export class WorkflowTextParameter extends Saveable implements Internal.WorkflowTextParameterProperties {
  ...
  constructor ({loadingOptions, extensionFields, id, label, doc, default_, position, optional, type, validators} : {loadingOptions?: LoadingOptions} & Internal.WorkflowTextParameterProperties) {
  ...
  /**
   * Used to construct instances of {@link WorkflowTextParameter }.
   *
   * @param __doc                           Document fragment to load this record object from.
   * @param baseuri                         Base URI to generate child document IDs against.
   * @param loadingOptions                  Context for loading URIs and populating objects.
   * @param docRoot                         ID at this position in the document (if available)
   * @returns                               An instance of {@link WorkflowTextParameter }
  ...
  static override async fromDoc (__doc: any, baseuri: string, loadingOptions: LoadingOptions,
    ...
    let type
    try {
      type = await loadField(_doc.type, LoaderInstances.typedslunionOfGalaxyTextTypeLoaderOrarrayOfunionOfWorkflowTextParameterLoader2,
        baseuri, loadingOptions)
    } catch (e) {
      if (e instanceof ValidationException) {
        __errors.push(
    ...
    if ('validators' in _doc) {
      try {
        validators = await loadField(_doc.validators, LoaderInstances.unionOfundefinedtypeOrarrayOfTextValidatorsLoader,
          baseuri, loadingOptions)
      } catch (e) {
        if (e instanceof ValidationException) {
          __errors.push(
            new ValidationException('the `validators` field is not valid because: ', [e])
          )
        } else {
          throw e
        }
    ...
    for (const [key, value] of Object.entries(_doc)) {
      if (!WorkflowTextParameter.attr.has(key)) {
        if ((key as string).includes(':')) {
          ...
          __errors.push(
            ...
            expected one of: \`label\`,\`doc\`,\`id\`,\`default\`,\`position\`,\`optional\`,\`type\`,\`validators\``)
          )
          break
        }
      }
    ...
    if (__errors.length > 0) {
      throw new ValidationException("Trying 'WorkflowTextParameter'", __errors)
    }

    const schema = new WorkflowTextParameter({
  ...
  : Dictionary<any> {
    ...
    if (this.validators != null) {
      r.validators = save(this.validators, false, this.id, relativeUris)
    }
  ...
  static attr: Set<string> = new Set(['label','doc','id','default','position','optional','type','validators'])
}
