...
def parser_info() -> str:
    return "org.galaxyproject.gxformat2.v19_09.draft"
...
class DraftWorkflowStep(WorkflowStep):
    """
    ...
    def __init__(
        ...
        runtime_inputs: Optional[Any] = None,
        when: Optional[Any] = None,
        _plan_state: Optional[Any] = None,
        _plan_context: Optional[Any] = None,
        _plan_in: Optional[Any] = None,
        _plan_out: Optional[Any] = None,
        extension_fields: Optional[dict[str, Any]] = None,
        loadingOptions: Optional[LoadingOptions] = None,
    ) -> None:
        if extension_fields:
        ...
        self.runtime_inputs = runtime_inputs
        self.when = when
        self._plan_state = _plan_state
        self._plan_context = _plan_context
        self._plan_in = _plan_in
        self._plan_out = _plan_out

    def __eq__(self, other: Any) -> bool:
        if isinstance(other, DraftWorkflowStep):
            return bool(
                self.id == other.id
                and self.label == other.label
                and self.doc == other.doc
                ...
                and self.runtime_inputs == other.runtime_inputs
                and self.when == other.when
                and self._plan_state == other._plan_state
                and self._plan_context == other._plan_context
                and self._plan_in == other._plan_in
                and self._plan_out == other._plan_out
            )
        return False
    ...
    def fromDoc(
        ...
        _plan_state = None
        if "_plan_state" in _doc:
            try:
                _plan_state = load_field(
                    _doc.get("_plan_state"),
                    union_of_None_type_or_strtype,
                    baseuri,
                    loadingOptions,
                    lc=_doc.get("_plan_state")
                )

            except ValidationException as e:
                error_message, to_print, verb_tensage = parse_errors(str(e))

                if str(e) == "missing required field `_plan_state`":
                    _errors__.append(
                        ValidationException(
                            str(e),
                            None
                    ...
                    val = _doc.get("_plan_state")
                    if error_message != str(e):
                        val_type = convert_typing(extract_type(type(val)))
                        _errors__.append(
                            ValidationException(
                                "the `_plan_state` field is not valid because:",
                                SourceLine(_doc, "_plan_state", str),
                                [ValidationException(f"Value is a {val_type}, "
                                                     f"but valid {to_print} for this field "
                                                     f"{verb_tensage} {error_message}",
                                                     detailed_message=f"Value `{val}` is a {val_type}, "
                        ...
                        _errors__.append(
                            ValidationException(
                                ...
                                SourceLine(_doc, "_plan_state", str),
                                [e],
                                detailed_message=f"the `_plan_state` field with value `{val}` "
                                "is not valid because:",
                            )
                        )
        _plan_context = None
        if "_plan_context" in _doc:
            try:
                _plan_context = load_field(
                    _doc.get("_plan_context"),
                    union_of_None_type_or_strtype,
                    baseuri,
                    loadingOptions,
                    lc=_doc.get("_plan_context")
                )

            except ValidationException as e:
                error_message, to_print, verb_tensage = parse_errors(str(e))

                if str(e) == "missing required field `_plan_context`":
                    _errors__.append(
                        ValidationException(
                            str(e),
                            None
                    ...
                    val = _doc.get("_plan_context")
                    if error_message != str(e):
                        val_type = convert_typing(extract_type(type(val)))
                        _errors__.append(
                            ValidationException(
                                "the `_plan_context` field is not valid because:",
                                SourceLine(_doc, "_plan_context", str),
                                [ValidationException(f"Value is a {val_type}, "
                                                     f"but valid {to_print} for this field "
                                                     f"{verb_tensage} {error_message}",
                                                     detailed_message=f"Value `{val}` is a {val_type}, "
                        ...
                        _errors__.append(
                            ValidationException(
                                ...
                                SourceLine(_doc, "_plan_context", str),
                                [e],
                                detailed_message=f"the `_plan_context` field with value `{val}` "
                                "is not valid because:",
                            )
                        )
        _plan_in = None
        if "_plan_in" in _doc:
            try:
                _plan_in = load_field(
                    ...
                    baseuri,
                    loadingOptions,
                    lc=_doc.get("_plan_in")
                )

            except ValidationException as e:
                error_message, to_print, verb_tensage = parse_errors(str(e))

                if str(e) == "missing required field `_plan_in`":
                    _errors__.append(
                        ValidationException(
                            str(e),
                            None
                    ...
                    val = _doc.get("_plan_in")
                    if error_message != str(e):
                        val_type = convert_typing(extract_type(type(val)))
                        _errors__.append(
                            ValidationException(
                                "the `_plan_in` field is not valid because:",
                                SourceLine(_doc, "_plan_in", str),
                                [ValidationException(f"Value is a {val_type}, "
                                                     f"but valid {to_print} for this field "
                                                     f"{verb_tensage} {error_message}",
                                                     detailed_message=f"Value `{val}` is a {val_type}, "
                        ...
                        _errors__.append(
                            ValidationException(
                                ...
                                SourceLine(_doc, "_plan_in", str),
                                [e],
                                detailed_message=f"the `_plan_in` field with value `{val}` "
                                "is not valid because:",
                            )
                        )
        _plan_out = None
        if "_plan_out" in _doc:
            try:
                _plan_out = load_field(
                    _doc.get("_plan_out"),
                    union_of_None_type_or_strtype,
                    baseuri,
                    loadingOptions,
                    lc=_doc.get("_plan_out")
                )

            except ValidationException as e:
                error_message, to_print, verb_tensage = parse_errors(str(e))

                if str(e) == "missing required field `_plan_out`":
                    _errors__.append(
                        ValidationException(
                            str(e),
                            None
                    ...
                    val = _doc.get("_plan_out")
                    if error_message != str(e):
                        val_type = convert_typing(extract_type(type(val)))
                        _errors__.append(
                            ValidationException(
                                "the `_plan_out` field is not valid because:",
                                SourceLine(_doc, "_plan_out", str),
                                [ValidationException(f"Value is a {val_type}, "
                                                     f"but valid {to_print} for this field "
                                                     f"{verb_tensage} {error_message}",
                                                     detailed_message=f"Value `{val}` is a {val_type}, "
                        ...
                        _errors__.append(
                            ValidationException(
                                ...
                                SourceLine(_doc, "_plan_out", str),
                                [e],
                                detailed_message=f"the `_plan_out` field with value `{val}` "
                                "is not valid because:",
                            )
                        )
        extension_fields: dict[str, Any] = {}
        for k in _doc.keys():
            if k not in cls.attrs:
                if not k:
                    ...
                    _errors__.append(
                        ValidationException(
                            "invalid field `{}`, expected one of: `id`, `label`, `doc`, `position`, `tool_id`, `tool_shed_repository`, `tool_version`, `errors`, `uuid`, `in`, `out`, `state`, `tool_state`, `post_job_actions`, `type`, `run`, `runtime_inputs`, `when`, `_plan_state`, `_plan_context`, `_plan_in`, `_plan_out`".format(
                                k
                            ),
                            SourceLine(_doc, k, str),
                        )
    ...
    def save(
        ...
        if self._plan_in is not None:
            r["_plan_in"] = save(
                self._plan_in, top=False, base_url=self.id, relative_uris=relative_uris
            )
        if self._plan_out is not None:
            r["_plan_out"] = save(
                self._plan_out, top=False, base_url=self.id, relative_uris=relative_uris
            )
...
class GalaxyWorkflowDraft(Process, HasUUID):
    ...
    def __init__(
        ...
        self.outputs = outputs
        self.uuid = uuid
        self.class_ = "GalaxyWorkflowDraft"
        self.steps = steps
        self.report = report
        self.tags = tags
        self.comments = comments
    ...
    def __eq__(self, other: Any) -> bool:
        if isinstance(other, GalaxyWorkflowDraft):
            return bool(
                self.id == other.id
                and self.label == other.label
                and self.doc == other.doc
    ...
    def fromDoc(
        ...
        try:
            ...
            class_ = load_field(
                _doc.get("class"),
                uri_GalaxyWorkflowDraft_classLoader_False_True_None_None,
                baseuri,
                loadingOptions,
                lc=_doc.get("class")
            )
        ...
        try:
            ...
            steps = load_field(
                _doc.get("steps"),
                idmap_steps_union_of_array_of_DraftWorkflowStepLoader,
                baseuri,
                loadingOptions,
                lc=_doc.get("steps")
            )
...
_vocab = {
    ...
    "DraftWorkflowStep": "https://galaxyproject.org/gxformat2/v19_09/draft#DraftWorkflowStep",
    ...
    "GalaxyWorkflowDraft": "https://galaxyproject.org/gxformat2/v19_09/draft#GalaxyWorkflowDraft",
...
_rvocab = {
    ...
    "https://galaxyproject.org/gxformat2/v19_09/draft#DraftWorkflowStep": "DraftWorkflowStep",
    ...
    "https://galaxyproject.org/gxformat2/v19_09/draft#GalaxyWorkflowDraft": "GalaxyWorkflowDraft",
...
DraftWorkflowStepLoader = _RecordLoader(DraftWorkflowStep, None, None)
GalaxyWorkflowDraftLoader = _RecordLoader(GalaxyWorkflowDraft, None, None)
...
GalaxyWorkflowDraft_classLoader = _EnumLoader(
    ("GalaxyWorkflowDraft",), "GalaxyWorkflowDraft_class"
)
uri_GalaxyWorkflowDraft_classLoader_False_True_None_None = _URILoader(
    GalaxyWorkflowDraft_classLoader, False, True, None, None
)
array_of_DraftWorkflowStepLoader = _ArrayLoader(DraftWorkflowStepLoader)
union_of_array_of_DraftWorkflowStepLoader = _UnionLoader(
    (array_of_DraftWorkflowStepLoader,)
)
idmap_steps_union_of_array_of_DraftWorkflowStepLoader = _IdMapLoader(
    union_of_array_of_DraftWorkflowStepLoader, "id", "None"
)
union_of_GalaxyWorkflowLoader_or_GalaxyWorkflowDraftLoader = _UnionLoader(
    (
        GalaxyWorkflowLoader,
        GalaxyWorkflowDraftLoader,
    )
)
array_of_union_of_GalaxyWorkflowLoader_or_GalaxyWorkflowDraftLoader = _ArrayLoader(
    union_of_GalaxyWorkflowLoader_or_GalaxyWorkflowDraftLoader
)
union_of_GalaxyWorkflowLoader_or_GalaxyWorkflowDraftLoader_or_array_of_union_of_GalaxyWorkflowLoader_or_GalaxyWorkflowDraftLoader = _UnionLoader(
    (
        GalaxyWorkflowLoader,
        GalaxyWorkflowDraftLoader,
        array_of_union_of_GalaxyWorkflowLoader_or_GalaxyWorkflowDraftLoader,
    )
)
...
def load_document(
    ...
    result, metadata = _document_load(
        union_of_GalaxyWorkflowLoader_or_GalaxyWorkflowDraftLoader_or_array_of_union_of_GalaxyWorkflowLoader_or_GalaxyWorkflowDraftLoader,
        doc,
        baseuri,
        loadingOptions,
    )
