...
from gxformat2.schema.gxformat2 import CreatorPerson, GalaxyType, GalaxyWorkflow as Format2LaxModel
...
def lint_ga(lint_context, nnw: NormalizedNativeWorkflow, raw_dict: dict | None = None):
    ...
    if raw_dict is not None:
        if "a_galaxy_workflow" not in raw_dict:
            ...
            lint_context.error(f"expected value [{raw_dict.get('a_galaxy_workflow')}] with key [a_galaxy_workflow] to be true")
    ...
    if step.type_ not in SKIP_DISCONNECTED_CHECK_TYPES_NATIVE:
        ...
        for input_def in step.inputs:
            if input_def.name and input_def.name not in input_connections:
                lint_context.warn(
                    f"Input {input_def.name} of workflow step {step_id} is disconnected."
                )
